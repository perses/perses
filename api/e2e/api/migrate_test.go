...
func TestMigrateEndpoint(t *testing.T) {
	testSuite := []struct {
	...
	}{
		{
			...
			initialDashboardPath: "api/plugin/migrate/testdata/simple_grafana_dashboard.json",
			resultDashboardPath:  "api/plugin/migrate/testdata/simple_perses_dashboard.json",
		},
		{
			title:                "grafana dashboard containing old-formatted elements (text panels without `options` field & a legacy graph panel)",
			initialDashboardPath: "api/plugin/migrate/testdata/old_grafana_panels_grafana_dashboard.json",
			resultDashboardPath:  "api/plugin/migrate/testdata/old_grafana_panels_perses_dashboard.json",
		},
	}
