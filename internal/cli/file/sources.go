...
func RetrieveFolderFromSources(folder string, destination string, maxFileSizeBytes int64, version string) error {
	...
	if err := downloadSources(version); err != nil {
		return fmt.Errorf("error downloading sources: %v", err)
	}
	defer cleanupSources()

	if err := extractSourcesFolderToDst(folder, destination, maxFileSizeBytes); err != nil {
...
// downloadSources downloads the sources of the given Perses release
...
// cleanupSources removes the local archive generated by downloadSources
func cleanupSources() {
...
// extractSourcesFolderToDst extracts the content of the given folder from the sources archive to the destination path
func extractSourcesFolderToDst(folder string, destination string, maxFileSizeBytes int64) error {
