...
func capturingMatrix(matrix model.Matrix, labelName string, capturingRegexp *v1.CapturingRegexp) []string {
	capturedData := make(map[string]bool)
	for _, sample := range matrix {
		if value, ok := sample.Metric[model.LabelName(labelName)]; ok {
			fillingCapturedData(string(value), capturedData, capturingRegexp.GetRegexp())
		}
	}
...
func (q *query) PromQLQuery(parameter *v1.PromQLQueryVariableParameter) *v1.VariableFeedResponse {
	...
	if matrix, ok := result.(model.Matrix); ok {
		return &v1.VariableFeedResponse{
			Name:   q.name,
			Values: capturingMatrix(matrix, parameter.LabelName, parameter.CapturingRegexp),
		}
	}
	return &v1.VariableFeedResponse{
		Name:   q.name,
