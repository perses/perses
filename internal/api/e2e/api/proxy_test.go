...
func TestSQLProxyBadMethod(t *testing.T) {
	e2eframework.WithServer(t, func(_ *httptest.Server, expect *httpexpect.Expect, manager dependency.PersistenceManager) []api.Entity {
		...
		expect.GET(fmt.Sprintf("/proxy/%s/%s/query", utils.PathGlobalDatasource, dtsName)).
			Expect().
...
func TestSQLProxyBadPath(t *testing.T) {
	e2eframework.WithServer(t, func(_ *httptest.Server, expect *httpexpect.Expect, manager dependency.PersistenceManager) []api.Entity {
		dtsName := "mySQLDTS"
		dts := newGlobalSQLDatasource(t, dtsName)
		e2eframework.CreateAndWaitUntilEntityExists(t, manager, dts)

		expect.POST(fmt.Sprintf("/proxy/%s/%s/badpath", utils.PathGlobalDatasource, dtsName)).
			Expect().
...
func TestSQLProxyGlobalDatasource(t *testing.T) {
	e2eframework.WithServer(t, func(_ *httptest.Server, expect *httpexpect.Expect, manager dependency.PersistenceManager) []api.Entity {
		...
		expect.POST(fmt.Sprintf("/proxy/%s/%s/query", utils.PathGlobalDatasource, dtsName)).
...
func TestSQLProxyProjectDatasource(t *testing.T) {
	e2eframework.WithServer(t, func(_ *httptest.Server, expect *httpexpect.Expect, manager dependency.PersistenceManager) []api.Entity {
		...
		expect.POST(fmt.Sprintf("/proxy/%s/%s/%s/%s/query", utils.PathProject, projectName, utils.PathDatasource, dtsName)).
			WithBytes([]byte(fmt.Sprintf(`{"query": "SELECT datname FROM pg_database"}`))).
			Expect().
			Status(http.StatusOK).
			Body().
...
func TestSQLProxyLocalDatasource(t *testing.T) {
	e2eframework.WithServer(t, func(_ *httptest.Server, expect *httpexpect.Expect, manager dependency.PersistenceManager) []api.Entity {
		...
		expect.POST(fmt.Sprintf("/proxy/%s/%s/%s/%s/%s/%s/query", utils.PathProject, projectName, utils.PathDashboard, dashboardName, utils.PathDatasource, dtsName)).
			WithBytes([]byte(fmt.Sprintf(`{"query": "SELECT datname FROM pg_database"}`))).
			Expect().
			Status(http.StatusOK).
			Body().
