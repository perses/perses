...
func signedToken(login, providerKind, providerID string, notBefore time.Time, expireAt time.Time, key []byte) (string, error) {
...
type JWT interface {
	SignedAccessToken(login, providerKind, providerID string) (string, error)
	SignedRefreshToken(login, providerKind, providerID string) (string, error)
...
func (j *jwtImpl) SignedAccessToken(login, providerKind, providerID string) (string, error) {
	now := time.Now()
	return signedToken(login, providerKind, providerID, now, now.Add(j.accessTokenTTL), j.accessKey)
}

func (j *jwtImpl) SignedRefreshToken(login, providerKind, providerID string) (string, error) {
	now := time.Now()
	return signedToken(login, providerKind, providerID, now, now.Add(j.refreshTokenTTL), j.refreshKey)
}
