...
func (s *sqlProxy) openMySQL(tlsConfig *tls.Config) (*sql.DB, error) {
	...
	// Use MariaDB config if driver is MariaDB, otherwise use MySQL config
	var driverConfig *datasourceSQL.MySQLConfig
	if s.config.Driver == datasourceSQL.DriverMariaDB && s.config.MariaDB != nil {
		driverConfig = s.config.MariaDB
	} else if s.config.MySQL != nil {
