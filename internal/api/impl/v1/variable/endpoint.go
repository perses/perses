...
func NewEndpoint(service variable.Service, readonly bool) *Endpoint {
	return &Endpoint{
...
func (e *Endpoint) CollectRoutes(g *shared.Group) {
	group := g.Group(fmt.Sprintf("/%s", shared.PathVariable))
	subGroup := g.Group(fmt.Sprintf("/%s/:%s/%s", shared.PathProject, shared.ParamProject, shared.PathVariable))
	if !e.readonly {
