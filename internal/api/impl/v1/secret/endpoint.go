...
func NewEndpoint(service secret.Service, rbacService authorization.RBAC, readonly bool) *Endpoint {
	return &Endpoint{
...
func (e *Endpoint) CollectRoutes(g *shared.Group) {
	group := g.Group(fmt.Sprintf("/%s", shared.PathSecret))
	subGroup := g.Group(fmt.Sprintf("/%s/:%s/%s", shared.PathProject, shared.ParamProject, shared.PathSecret))
	if !e.readonly {
