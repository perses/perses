...
func NewEndpoint(service role.Service, rbacService authorization.RBAC, readonly bool) *Endpoint {
	return &Endpoint{
		toolbox:  shared.NewToolBox(service, rbacService, v1.KindRole),
		readonly: readonly,
	}
}

func (e *Endpoint) CollectRoutes(g *shared.Group) {
	group := g.Group(fmt.Sprintf("/%s", shared.PathRole))
	subGroup := g.Group(fmt.Sprintf("/%s/:%s/%s", shared.PathProject, shared.ParamProject, shared.PathRole))
	if !e.readonly {
