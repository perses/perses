...
A dashboard can be created through the UI or managed [as-code](./dashboard-as-code.md) and necessarily belongs to a [project](./project.md).
...
A dashboard's visual elements consist in a set of panels. Each panel displays specific data in the form of charts, tables, or other visualization types, allowing users to quickly interpret metrics, traces, logs or profiles. Panels are plugged to the datasources via their configured queries, that are written in the relevant datasource’s query language and specify the exact data to retrieve from this datasource. These queries can also make use of the configured variables to allow dynamic query scope for the end user.
