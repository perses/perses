...
An empty list leaves headers unchanged. The two lists cannot both be non-empty.
Filtering happens after configured headers and secret authentication are added, so include `Authorization`
in `allowHeaders` when the datasource requires secret authentication. Excluding `X-Forwarded-For` also prevents
the reverse proxy from adding it. The target `Host` and headers required or generated by the HTTP transport
