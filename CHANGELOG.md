...
This new release introduces a complete review of the migration script.
Performance improvement and reducing the chaos generated by this script were the key of rewriting the script.

We also provide many updates about the migration itself,
covering more use cases raised by the community (and also fixed thanks to you).

Then we are starting to implement some commands in the CLI for future plugins' development.
This is part of the new plugin design we are working on.

Finally, thanks to the community, we have a new panel named `Status History` and the `Table` panel has been improved
