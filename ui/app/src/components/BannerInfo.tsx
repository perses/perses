...
export function BannerInfo(): ReactElement | null {
  ...
  const isBannerEnabled = useMemo(() => banner?.information !== discardedBanner,
    [banner?.information, discardedBanner]
  );
  ...
  return (
    <Box >
      <Alert sx={{ '& .MuiAlert-message': { '& h1': { fontSize: (theme) => theme.typography.h2.fontSize, margin: (theme) => theme.spacing(0.5) } } }}
        severity={banner.severity} onClose={() => setDiscardedBanner(banner.information)}>
        <span dangerouslySetInnerHTML={{ __html: banner.information }}></span>
      </Alert>
    </Box>
  );
}
