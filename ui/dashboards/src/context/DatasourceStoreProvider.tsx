...
export function DatasourceStoreProvider(props: DatasourceStoreProviderProps): ReactElement {
  ...
  const [localDatasources, setLocalDatasources] = useState<GenericDatasourceResource[]>([]);
  ...
  const findDatasource = useEvent(async (selector: DatasourceSelector) => {
    const searchDataset = [...datasources, ...localDatasources];
  ...
  const queryDatasources = useCallback(
    (selector: DatasourceSelector) => {
      ...
      const queryDataSet = [...datasources, ...localDatasources];
  ...
  const addToLocalDatasources = useCallback(
    (newLocalDatasources: GenericDatasourceResource[]) => {
      const uniqueNewLocalDatasources = newLocalDatasources.filter((newLocalDatasource) => {
        for (const localDatasource in localDatasources) {
          if (isEqual(localDatasource, newLocalDatasource)) {
            return false;
          }
        }
        return true;
      });
      setLocalDatasources([...localDatasources, ...uniqueNewLocalDatasources]);
    },
    [localDatasources]
  );

  const clearLocalDatasources = useCallback(() => {
    setLocalDatasources([]);
  }, []);
