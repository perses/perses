...
import { usePlugin, DEFAULT_ALL_VALUE, useDatasourceStore } from '@perses-dev/plugin-system';
...
function ListVariable({ name }: TemplateVariableProps) {
  ...
  const loadOptions = useCallback(async () => {
    ...
    try {
      const { data } = await variablePlugin.getVariableOptions(definition, { datasourceStore });
      setVariableOptions(name, data);
    } catch (e) {
      console.error('Failed to load template variable options', e);
    }
    setVariableLoading(name, false);
  }, [variablePlugin, definition, name, setVariableLoading, setVariableOptions, datasourceStore]);
