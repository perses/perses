...
import { checkforNearbyTimeSeries } from './focused-series';

describe('checkforNearbyTimeSeries', () => {
  ...
  it('should return focused series data for points nearby the cursor', () => {
    ...
    expect(checkforNearbyTimeSeries(chartData, pointInGrid, yBuffer, decimalUnit)).toEqual(focusedSeriesOutput);
  });
  ...
  it('should return series values formatted as a percent', () => {
    ...
    expect(checkforNearbyTimeSeries(chartData, pointInGrid, yBuffer, percentFormattedUnit)).toEqual(
