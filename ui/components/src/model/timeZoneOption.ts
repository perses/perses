...
export const getTimeZoneOptions = memoize((): TimeZoneOption[] => {
  ...
  // Add local timezone to the list if it's not already there
  const timezonesToUse = COMMON_TIMEZONES.includes(localTimeZone)
    ? COMMON_TIMEZONES
    : [localTimeZone, ...COMMON_TIMEZONES];

  return (
    // putting default and UTC options first for better user experience
    [DEFAULT_DASHBOARD_TIMEZONE, 'UTC', ...timezonesToUse]
      .map((timeZone) => {
